  ...
  module Receptor
    class ApiObject
      ...
      def check_kafka_response(response)
        ...
        raise TopologicalInventory::MigrationToolkitVirtualization::Receptor::ReceptorNodeError.new, "#{msg}, response: #{response}" if response.kind_of?(String)
        # Non-hash, non-string response means unknown error
        raise TopologicalInventory::MigrationToolkitVirtualization::Receptor::ReceptorUnknownResponseError.new, "#{msg}, response: #{response.inspect}" unless response.kind_of?(Hash)

        # Non-standard hash response
        raise TopologicalInventory::MigrationToolkitVirtualization::Receptor::ReceptorKafkaResponseError.new, "#{msg}, response: #{response.inspect}" if response['status'].nil? || response['body'].nil?

        status = response['status'].to_i
        if status < 200 || status >= 300
          # Bad response error
          msg = "Response from #{uri} failed: HTTP status: #{response['status']}"
          raise TopologicalInventory::MigrationToolkitVirtualization::Receptor::ReceptorKafkaResponseError.new, msg
        end
      end
